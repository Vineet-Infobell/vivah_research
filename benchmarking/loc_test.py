...
# ========================
# 2. CORPUS (Indian Locations Database)
# ========================
CORPUS_LOCATION = [
...
print(f"✅ Corpus Loaded: {len(CORPUS_LOCATION)} Unique Locations")
...

{
 ...
 "ground_truth": {
     "tier_1": ["NCR"],
     "tier_2": ["Delhi"],
     "tier_3": ["Gurgaon", "Noida", "Ghaziabad", "Faridabad"]
 }
},
...

{
 ...
 "ground_truth": {
     "tier_1": ["South India"],
     "tier_2": ["Bengaluru", "Chennai", "Hyderabad"],
     "tier_3": ["Kochi", "Coimbatore", "Mysore"]
 }
},
...
def calculate_metrics(retrieved_locs, ground_truth_tiered, k=5):
    """
    Calculate weighted metrics based on tiered ground truth.
    
    Args:
        retrieved_locs: List of retrieved job titles (ordered by model)
    ...
    mrr = 0
    for i, job in enumerate(retrieved_locs):
...
<head>
    ...
    <title>Location Embedding Benchmark (India) Report</title>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🏆 Location Embedding Benchmark (India) Report</h1>
...
def run_benchmark():
    ...
    for config in CONFIGS_TO_TEST:
        ...
        try:
            doc_resp = client.models.embed_content(
                model=model_id, contents=CORPUS_LOCATION,
        ...
        for scenario in TEST_SCENARIOS:
            ...
            try:
                ...
                for dist_name, scores in distances.items():
                    ...
                    retrieved_locs = [CORPUS_LOCATION[i] for i in top_indices]
                    
                    p, r, mrr, ndcg = calculate_metrics(retrieved_locs, gt_tiered, k=5)
    ...
    # ========================
    json_path = Path(__file__).parent / "loc_results.json"
    ...
    report_path = Path(__file__).parent / "loc_benchmark_report.html"
