"""
APPROACH 5: Minimal Hard Filters + Semantic + Cross-Encoder
------------------------------------------------------------
Flow:
1. Hard Filters (Minimal): 
   - Gender (Exact)
   - Religion (Exact)
   - NO Age Filter
   - NO Location Filter
2. Semantic Search: Get top 50 candidates
3. Cross-Encoder Re-ranking: Deep semantic understanding
4. Top 10 Results (sorted by CE score only)

Note: Pure cross-encoder ranking without age scoring
"""
...
print("="*80)
print("🎯 APPROACH 5: Minimal Filters + Semantic + Cross-Encoder")
print("="*80)
...
def search_matches(query: str, filters: dict, criteria: dict, searcher: dict, top_k: int = 10):
    """
    Search using minimal hard filters + semantic + cross-encoder
    """
    ...
    # BUILD SQL QUERY - ONLY Gender and Religion filters
    where_clauses = [
    ...
    for candidate in candidates:
        ...
        candidate_text = f"{job_title}, {education}, {location}, {age} years old"
    ...
    # Sort by CE score and take top K
    results.sort(key=lambda x: x['ce_score'], reverse=True)
    results = results[:top_k]
    
    total_time = (time.time() - start_time) * 1000
    
    return results, total_time, emb_time, db_time, ce_time
...
def calculate_ndcg(predicted_ids, ground_truth_matches, k=10):
    ...
    relevance_map = {m['user_id']: m.get('relevance_score', 3) for m in ground_truth_matches}
    ...
    ideal_relevance = sorted([m.get('relevance_score', 3) for m in ground_truth_matches], reverse=True)[:k]
...
for scenario in ground_truth:
    ...
    # Search
    matches, latency, emb_time, db_time, ce_time = search_matches(
        scenario['query'],
        scenario['filters'],
        scenario['criteria'],
        scenario['searcher'],
...
output = {
    "approach": "Approach 5: Minimal Filters + Semantic + Cross-Encoder",
    "configuration": {
...
with open("approach_5_results.json", 'w', encoding='utf-8') as f:
    json.dump(output, f, indent=2, ensure_ascii=False)

print(f"\n✅ JSON Results saved to: approach_5_results.json")
...
<html>
<head>
    <title>Matchmaking Report - Approach 5</title>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .stat-card .value {{ font-size: 3em; font-weight: bold; color: #3b82f6; }}
        ...
        .config-item .config-label {{ font-weight: bold; color: #3b82f6; margin-bottom: 5px; }}
        ...
        .chart-container h2 {{ color: #3b82f6; margin-bottom: 20px; }}
        ...
        .scenario-header h2 {{ color: #3b82f6; font-size: 1.8em; margin-bottom: 10px; }}
        ...
        .profile-card.matched {{ border-color: #3b82f6; background: #eff6ff; }}
...
<div class="container">
    <div class="hero">
        <h1>🎯 Approach 5: Minimal Filters + Semantic + Cross-Encoder</h1>
        <p class="subtitle">Only Gender + Religion filters → Semantic search (Top 50) → Pure cross-encoder re-ranking</p>
    ...
    <div class="config-box">
        ...
        <div class="config-grid">
            ...
            <div class="config-item">
                ...
                <div class="config-value">Pure Cross-Encoder (100%)</div>
            </div>
            <div class="config-item">
                <div class="config-label">Candidates Retrieved</div>
                <div class="config-value">Top 50 → Top 10</div>
            </div>
        </div>
        <p style="margin-top: 20px; color: #666; line-height: 1.8;">
            <strong>Approach:</strong> This method applies minimal hard filters (only gender and religion), allowing 
            maximum flexibility for semantic matching. It retrieves top 50 candidates via semantic search, then uses 
            cross-encoder for deep re-ranking (100%). No age scoring or additional weighting is applied.
            This approach maximizes semantic relevance without strict age or location constraints.
        </p>
    </div>
...
for i, result in enumerate(results):
    ...
    <div class="scenario">
        <div class="scenario-header">
            ...
            <p><strong>Filters:</strong> {scenario['filters']['gender']}, {scenario['filters']['religion']}{age_info} (No hard age/location filters applied)</p>
        </div>
        ...
        html_content += f"""
                <div class="profile-card {card_class}">
                    ...
                    <div class="score-pills">
                        <span class="score-pill">CE Score: {profile['ce_score']:.3f}</span>
...
html_content += f"""
    <div class="insights-box">
        <h2>💡 Key Insights</h2>
        <ul>
            <li><strong>Minimal Filtering:</strong> Only gender and religion filters applied - maximum flexibility for semantic matching</li>
            <li><strong>No Age/Location Hard Filters:</strong> Allows cross-encoder to find semantically relevant matches regardless of age or location</li>
            <li><strong>Pure Cross-Encoder:</strong> 100% cross-encoder ranking provides superior semantic understanding vs pure cosine similarity</li>
            <li><strong>No Age Bias:</strong> Results purely based on semantic relevance without age preferences</li>
            <li><strong>Performance:</strong> Average NDCG of {avg_ndcg:.3f} with {avg_precision:.1f}% precision in {avg_latency:.0f}ms</li>
            <li><strong>Trade-off:</strong> More candidates to process but better semantic relevance due to minimal filtering</li>
        </ul>
    </div>
</div>
...
with open("approach_5_report.html", "w", encoding='utf-8') as f:
    f.write(html_content)

print("✅ Detailed HTML Report saved to: approach_5_report.html")
